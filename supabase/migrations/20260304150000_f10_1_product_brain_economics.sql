-- =============================================================================
-- F10.1 Product Brain — Economics (product-level profit KPIs)
-- =============================================================================
-- RPCs per Project Detail Economics tab: KPI range + series per product.
-- =============================================================================
...
BEGIN
  ...
  INTO v_ledger_total, v_allocated, v_unallocated
  FROM public.v_profit_allocation_coverage cov
  WHERE cov.org_id = p_org_id
    AND cov.d >= p_from AND cov.d <= p_to;

  v_incomplete := (v_unallocated > 0)
    OR (COALESCE(v_allocated, 0) * 100.0 / NULLIF(COALESCE(v_ledger_total, 0), 0) < 100)
    OR NOT EXISTS (
...
AS $$
  ...
  LEFT JOIN public.v_profit_allocation_coverage cov
    ON cov.org_id = p.org_id AND cov.d = p.d
  WHERE p.org_id = p_org_id AND p.product_id = p_product_id
    AND p.d >= p_from AND p.d <= p_to
  ORDER BY p.d;
$$;
