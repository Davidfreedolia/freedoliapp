...
- Taula **product_identifiers** passa a tenancy per **org_id** (UNIQUE(org_id, project_id)), RLS `is_org_member(org_id)`.
- Trigger omple `org_id` des de `projects` si és NULL en insert/update.

---

## S1.4b — product_identifiers purge legacy

- **is_demo**: columna eliminada.
- **user_id**: es manté només per audit (nullable, sense default); eliminats índexs/UNIQUE que en depenien.
- RLS: només policy org-based; eliminades policies user-based residuals.
- Contracte final: product_identifiers és org-scoped; sense is_demo; user_id opcional (auditoria).
